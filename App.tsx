...
const App: React.FC = () => {
  ...
  const [level, setLevel] = useState<string>(() => {
    ...
    // Ensure we validate against the currently loaded targetLang
  ...
  const handleTargetLangChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // When target language changes, reset level to the first option of that language
  ...
  const handleVoicePreview = async () => {
    ...
    // --- 1. System Voice (Instant, no caching needed) ---
    if (voice === 'system') {
      setIsPreviewLoading(true); // Short flicker to show interaction
      window.speechSynthesis.cancel();
      // Speak the target language greeting
    ...
    // --- 2. Gemini Voice (Check Hardcoded Cache First) ---
    // Cache key relies on Target Language now
    ...
    // --- 3. Gemini Voice (API Fallback) ---
    try {
      setIsPreviewLoading(true);
      const textToSay = PREVIEW_MESSAGES[targetLang];
      const base64Audio = await synthesizeSpeech(textToSay, voice);
      
      // Developer Helper: Log the base64 so it can be copied to voicePreviews.ts
    ...
    <div className="min-h-screen pb-12">
      ...
      <header className="bg-white border-b-4 border-black sticky top-0 z-50 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             <div className="hidden sm:block px-4 py-1.5 bg-sky-200 border-2 border-black rounded-full text-sm font-bold text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
              {t.tagline}
            </div>
            <button 
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className={selectClass}
                  >
                    {LEVEL_OPTIONS[targetLang].map((opt) => (
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-white border-2 border-black rounded-xl px-4 flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all hover:bg-slate-50"
                    title="Preview Voice"
                  >
                     {isPreviewLoading ? (
              ...
              <div className="pt-4">
                <button 
                  onClick={() => setIsSettingsOpen(false)}
                  className="w-full bg-black text-white font-bold py-3 rounded-xl shadow-[4px_4px_0px_0px_rgba(100,100,100,1)] hover:bg-slate-800 active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_rgba(100,100,100,1)] transition-all"
                >
                  {t.closeButton}
                </button>
              </div>
            ...
            {!imageBase64 ? (
              ...
              <div className="relative rounded-3xl overflow-hidden border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] group bg-white">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                  <button 
                    onClick={reset}
                    className="bg-red-500 hover:bg-red-400 text-white border-2 border-black px-6 py-3 rounded-2xl font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all"
                  >
                    {t.removeButton}
                  </button>
                </div>
